...
include_once("adm/model/m_grado_instruccion.php");
...
$lista = $obj_grado_instruccion->lis_grado_instruccion(0);
?>
<script>

    function adicionar(id_pagina) {
        var p_accion = "I";
        $.post("p_grado_instruccion.php", {f: 'PopupParametro'}, function (data) {
            carga_modal(vg_dlg, data, false, true, 660, '::: Registrar Grado de Instrucción :::', ['center', 120], [
    ...
    function cargar_data(codigo) {
        $.post('adm/controller/c_grado_instruccion.php', {
            p_id_grado_instruccion: codigo,
            p_accion: "B_UNO",
            p_nombre: "",
            p_estado: "A"
        }, function (data) {
            var res = data.split(",");
            $('#p_id_grado_instruccion').val(res[0]);
            $('#p_nombre').val(res[1]);
            if (res[2] == "A") {
    ...
    function modificar(p_id, id_pagina) {
        ...
        $.post("p_grado_instruccion.php", {f: 'PopupParametro'}, function (data) {
            cargar_data(p_id);
            carga_modal(vg_dlg, data, false, true, 660, '::: Modificar Grado de Instrucción:::', ['center', 120], [{
                    text: "Aceptar", click: function () {
                        adicionar_nuevo(p_accion, id_pagina);
                    }
                }, {text: "Cerrar", click: function () {
...
<div class="row">
    <div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
        ...
        <?php if ($flag_nuevo == 1) { ?>
            <button href='javascript:void(0);' onclick='adicionar(<?php echo $id_pagina; ?>);' class="btn btn-primary" rel="tooltip" data-placement="top" data-original-title="agregar"><i class="fa fa-plus-circle"></i> Nuevo Grado de Instruccion</button>
        <?php } ?>
                        ...
                        <table id="datatable_tabletools" class="table table-striped table-bordered table-hover" width="100%">
                            ...
                            <tbody>
                                ...
                                for ($i = 0; $i < count($lista); $i++) {
                                    $cod_grado_instruccion = $lista[$i]["id_grado_instruccion"];
                                    $nombre = $lista[$i]["nombre"];
                                    $ind_estado = $lista[$i]["estado"];
                                    ...
                                    <tr>
                                        <td align='center' width='10%'><?php echo $cod_grado_instruccion; ?> </td>
                                        <td align='left' width='45%'><?php echo $nombre; ?></td>
                                        
                                        <td align='center' width='20%'><?php echo $msg_estado; ?></td>
                                        <td align='center' width='25%'>
                                            <?php if ($flag_modif == 1) { ?>
                                                <a class="btn btn-xs btn-primary" rel="tooltip" data-placement="top" data-original-title="editar" href='javascript:void(0);' onclick='modificar(<?php echo $cod_grado_instruccion; ?>,<?php echo $id_pagina; ?>);' ><i class="fa fa-pencil"></i></a> 
                                                <?php
                                            }
                                            if ($flag_elim == 1) {
                                                ?> 
                                                <a class="btn btn-xs btn-danger" rel="tooltip" data-placement="top" id="perrito" data-original-title="eliminar" href='javascript:void(0);' onclick='preguntar_eliminar(<?php echo $cod_grado_instruccion; ?>,<?php echo $id_pagina; ?>);' ><i class="fa fa-times"></i></a> 
